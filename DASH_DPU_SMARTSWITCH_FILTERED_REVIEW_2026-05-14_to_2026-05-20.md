...
- Community meeting discussed **64 PRs created / 45 merged** over the cumulative period ending this week (72.6% merge rate); top-10 most-impactful PRs presented to the group
...
> ⚠️ **AI-Generated Notes Disclaimer:** The following meeting notes were generated by AI transcription. Content has been cross-referenced with PR data but **should be verified for accuracy** before relying on them.

### PR Creation and Merge Metrics (Meeting Discussion)

Kristina reported on the cumulative period ending this week:
- **64 PRs created**, **45 PRs merged**, **72.6% merge rate**
- **sonic-mgmt** was highlighted as the area with the highest creation and merge activity
- A **top-10 most impactful PRs** list was presented to the group (see Highlights below)
- Detailed PR lists were shared in meeting notes, with an offer to remove them if the email becomes too lengthy

### Notable Merged PRs Discussed in Meeting

| PR | Discussion Summary | Contributor(s) |
|----|--------------------|----------------|
| sonic-mgmt [#24046](https://github.com/sonic-net/sonic-mgmt/pull/24046) – Update SmartSwitch HA test plan | HA test plan update merged; HA work by Jing, Michael Aronovici, and Fred called out | Jing, Michael Aronovici, Fred |
| sonic-dash-api [#67](https://github.com/sonic-net/sonic-dash-api/pull/67) – Update key ordering in ha_scope_config | Updates to key ordering and mapping counters/table names highlighted | Lawrence |
| sonic-gnmi [#665](https://github.com/sonic-net/sonic-gnmi/pull/665) – Add DPU_COUNTERS_DB virtual path handling | DPU counters and DB virtual path handling merged | Lawrence |
| sonic-mgmt [#23915](https://github.com/sonic-net/sonic-mgmt/pull/23915) – Add utility to upgrade DPU images | DPU image upgrade utility for smart switch test bed merged; resolved issue where only NPU was upgraded during nightly runs | Lawrence |

### Draft / Open PRs Discussed in Meeting

| PR | Discussion Summary | Status |
|----|--------------------|-|
| sonic-sairedis [#1897](https://github.com/sonic-net/sonic-sairedis/pull/1897) – Improve smartswitch memory scaling | Conditionally caches high-volume DASH objects; **30–40% memory reduction** observed on SONiC VS virtual test bed. Focus is memory improvement (not speed). Partial caching applied to select DASH objects. Before-and-after results to be added. Contributor: **N. Hegde** (new SONiC team member), context from **Prabhat** | Draft |
| sonic-SONiC [#2336](https://github.com/sonic-net/SONiC/pull/2336) – HA bulk sync failure handling | Open PR by **Changrong** defining bulk sync failure rollback to standalone mode. Still under construction | Open |
| sonic-swss-common [#1188](https://github.com/sonic-net/sonic-swss-common/pull/1188) – Expose batched ZmqProducerStateTable APIs | Enhances ZMQ producer state table APIs to enable batching for multiple entries per table, reducing end-to-end programming time by avoiding per-entry updates | Open |
...
**Memorial Day Scheduling:** Kristina checked in on upcoming meeting availability. murali Venkateshaiah (muraliv) and others confirmed availability around Memorial Day.
