...
![dash-layered-architecture](images/dash-software-stack.svg)

<figcaption><i>DASH software stack</i></figcaption><br/> 
...
The PCIe bus _can_ be used to bootstrap/upgrade cards and perform some platform management functions but is not a participant in steady-state datacenter traffic.
...
1. A _new docker container_ in the user space named **dash container** to create the functional component for DASH.

1. In the **sync-d container**, the **sai api DASH** (as opposed to _sai api_ in the original SONiC architecture).  

The _DPU/IPU/SmartNic_ hardware will run a separate instance of SONiC-DASH on the hardware.  
...
The functionality of the new _dash container_ in the user space is to receive content from the Software Defined Networking (SDN) controller to control setup for the overlay configurations. DASH receives the objects, translates them with a **gNMI agent**, provides them to the _SONiC OrchAgent_ for further translation onto the dataplane via the **SAI database**.
